		...
		<div class="col-lg-10" align="center" style="float: right;">
			...
			<table class="table table-striped" style="text-align: center;">
				...
				<tr>
					...
					href='branchupdate.jsp?branchId=<%=b.getBranchId()%>' >Update</button></td>
			...
			<nav aria-label="Page navigation">
				<ul class="pagination">
					...
					%>
					<li><a
						href="administrationBranchUpdate?pageNo=<%=(pageNo - 1)%>&pageSize=<%=pageSize%> "
					...
					%>
					<li><a
						href="administrationBranchUpdate?pageNo=<%=i%>&pageSize=<%=pageSize%>"><%=i%></a></li>
					<%
						}
					...
					%>
					<li><a
						href="searchBranchUpdate?searchString=<%=searchString%>&pageNo=<%=(pageNo - 1)%>&pageSize=<%=pageSize%>"
						aria-label="Previous"> <span aria-hidden="true">&laquo;</span>
					</a></li>
					<%
						}
					...
					%>
					<li><a
						href="searchBranchUpdate?searchString=<%=searchString%>&pageNo=<%=i%>&pageSize=<%=pageSize%>"><%=i%></a></li>
					<%
						}
							if (pageNo != totalPages) {
					%>
