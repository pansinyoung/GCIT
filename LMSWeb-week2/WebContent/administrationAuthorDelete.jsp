...
<%
	...
	int pageNo = 1;
	int pageSize = 10;
	String searchString = "Search author!";
	...
	if (request.getAttribute("authors") != null) {
		...
		authors = adminService.readAllAuthor(1, 10);
		totalPages = adminService.getAllCountAuthor();
	}
...
<div class="container theme-showcase" role="main">
	...
	<div class="col-md-6" align="right">
		<form class="form-inline"
			action="searchAuthorsDelete?pageNo=<%=pageNo%>&pageSize=<%=pageSize%>"
			method="get">
			<div class="row">
	...
	<div class="row">
		...
		<div class="col-lg-10" align="center" style="float: right;">
			...
			<table class="table table-striped" style="text-align: center;">
				...
				<%
					for (Author a : authors) {
						List<Book> books = adminService.viewAuthorBooks(a.getAuthorId());
				%>
				<tr>
					...
					<td><button class="btn btn-sm btn-danger" onclick="javascript:location.href='administrationAuthorDelete?authorId=<%=a.getAuthorId()%>'">Delete</button></td>
				</tr>
				<%
					}
				%>
			...
			<nav aria-label="Page navigation">
				<ul class="pagination">
					<%
						if (searchString == "Search author!") {
							if (pageNo != 1) {
					%>
					<li><a
						href="administrationAuthorDelete?pageNo=<%=(pageNo - 1)%>&pageSize=<%=pageSize%> "
					...
					%>
					<li><a
						href="administrationAuthorDelete?pageNo=<%=i%>&pageSize=<%=pageSize%>"><%=i%></a></li>
					<%
						}
					...
					%>
					<li><a
						href="searchAuthorsDelete?searchString=<%=searchString%>&pageNo=<%=(pageNo - 1)%>&pageSize=<%=pageSize%>"
						aria-label="Previous"> <span aria-hidden="true">&laquo;</span>
					</a></li>
					<%
						}
					...
					%>
					<li><a
						href="searchAuthorsDelete?searchString=<%=searchString%>&pageNo=<%=i%>&pageSize=<%=pageSize%>"><%=i%></a></li>
					<%
						}
							if (pageNo != totalPages) {
					%>
